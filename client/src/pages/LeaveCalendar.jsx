...
export default function LeaveCalendar() {
      ...
      if (leavesOnDate.length > 0) {
        return (
          <div className="flex justify-center items-center h-full">
            <div className="w-2 h-2 bg-red-500 rounded-full" title={leavesOnDate.map(l => l.employeeId).join(', ')}></div>
          </div>
        );
      }
    }
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 h-screen overflow-y-auto">
      <h1 className="text-2xl sm:text-3xl font-semibold mb-6 text-slate-800">Leave Calendar</h1>
      ...
      <div className="bg-slate-100 p-4 sm:p-6 rounded-lg shadow-lg">
        <Calendar
        ...
        <div className="mt-6 bg-slate-50 p-4 rounded-lg shadow">
          ...
          <ul className="space-y-2">
            {selectedDateLeaves.map(leave => (
              <li key={leave._id} className="p-3 bg-white rounded-md shadow-sm border border-slate-200">
                <p className="font-medium text-slate-700">
                  Employee ID: <span className="font-normal text-slate-600">{leave.employeeId}</span>
                </p>
                <p className="font-medium text-slate-700">
                  Duration: <span className="font-normal text-slate-600">
                    {new Date(leave.fromDate).toLocaleDateString()} - {new Date(leave.toDate).toLocaleDateString()}
                  </span>
                </p>
                {leave.reason && (
                  <p className="font-medium text-slate-700">
                    Reason: <span className="font-normal text-slate-600">{leave.reason}</span>
                  </p>
                )}
              </li>
            ))}
